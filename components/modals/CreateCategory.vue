			...
			<v-row class="bg-cyan-darken-4 pa-0 ma-0 pt-3">
				<v-col cols="12">
					<div class="d-flex justify-space-between">
						<div class="text-h6">
							<v-icon class="mr-4">mdi-shape-plus-outline</v-icon>
							Create category
						</div>
...
const { isOpenModal, selectedCategory } = defineProps({
	isOpenModal: false,
...
const handleFormSubmit = handleSubmit((values) => {
	...
	if (selectedCategory) {
		values["parent"] = selectedCategory.id;
	}
	...
	productStore
		...
		.then((d) => {
			loading.value = false;
			close();
			toast.success("Succesfully create the category!");
		})
		.catch((e) => {
			loading.value = false;
			console.log(e);
