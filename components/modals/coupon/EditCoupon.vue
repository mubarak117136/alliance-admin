...
const handleFormSubmit = handleSubmit((values) => {
	...
	utilityStore
		...
		.then((d) => {
			loading.value = false;
			close(true);
			toast.success("Succesfully updated coupon!");
		})
		.catch((e) => {
			loading.value = false;
			console.log(e);
...
onMounted(() => {
	...
	if (data) {
		formData.name.value.value = data?.name;
		formData.code.value.value = data?.code;
		formData.min_order_value.value.value = data?.min_order_value;
		formData.price_type.value.value = data?.price_type;
		formData.discount_amount.value.value = data?.discount_amount;
		formData.is_active.value.value = data?.is_active;
		formData.quantity.value.value = data?.quantity;
	}
});
</script>
