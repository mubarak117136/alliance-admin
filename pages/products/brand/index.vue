<template>
	<div>
		<v-row class="pa-0 ma-0 pl-0 pr-0 pl-md-5 pr-md-5">
			<v-col cols="12" class="d-flex flex-column ga-5">
				<v-sheet rounded="lg" class="pa-4 d-flex justify-space-between">
					<div>
						<div class="text-surface-bright">Brands ({{ totalCount }})</div>
					</div>
					<v-btn
						variant="tonal"
						size="small"
						@click="createBrandModalOpen = true"
					>
						<v-icon color="info" size="x-large">mdi-plus-thick</v-icon>
						<v-tooltip activator="parent" location="bottom"
							>Create Brand</v-tooltip
						>
					</v-btn>
				</v-sheet>
				<v-sheet rounded="lg" class="pa-4 d-flex justify-space-between">
				...
				<v-sheet rounded="lg" class="pa-4">
					<v-table density="compact" v-if="brands.length > 0">
						<thead>
						...
						<tbody>
							<tr v-for="(brand, idx) in brands" :key="'brand_' + idx">
								<td class="text-surface-bright">{{ brand.name }}</td>
								<td class="text-surface-bright">{{ brand.slug }}</td>
		...
		<ModalsCreateBrand
			v-if="createBrandModalOpen"
			:isOpenModal="createBrandModalOpen"
			@close="handleCreateBrandClose"
		/>
	</div>
</template>
...
var brands = ref([]);
const createBrandModalOpen = ref(false);
...
const handleCreateBrandClose = (refresh = false) => {
	createBrandModalOpen.value = false;
	if (refresh == true) {
		handleFetchBrands();
	}
};
